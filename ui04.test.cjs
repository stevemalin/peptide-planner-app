...
test('inventory cards expose direct add and compact missing low deficit warnings',()=>{const plans=fs.readFileSync('./app/src/MyPlans.tsx','utf8');assert.match(plans,/Add inventory for /);for(const label of ['Inventory not entered','Inventory deficit','Low inventory','More inventory needed'])assert.match(plans,new RegExp(label));assert.match(plans,/supplyWarning/);});

test('unreadable local data opens a non-destructive backup recovery path',()=>{const app=fs.readFileSync('./app/App.tsx','utf8'),store=fs.readFileSync('./app/src/store.ts','utf8');assert.match(app,/if\(saved\.loadFailed\)return/);assert.match(app,/Your saved planner data could not be opened/);assert.match(app,/saved\.loadFailed\?saved\.recover/);assert.match(store,/const recover=async/);assert.match(store,/loadFailed\.current=false/);});
...
test('published research cards distinguish cited protocols from recommendations and gate transfer',()=>{const practice=fs.readFileSync('./app/src/research-practice.ts','utf8'),card=fs.readFileSync('./app/src/ResearchPracticeCard.tsx','utf8');assert.match(practice,/PUBLISHED RESEARCH REFERENCE/);assert.match(practice,/specific population or species, purpose, route and formulation/);assert.match(practice,/not a personalized recommendation/);assert.match(practice,/does not establish safety or effectiveness/);assert.match(practice,/explicitly approved for transfer/);assert.match(card,/Use reviewed reference in Guide/);assert.match(card,/No reviewed transfer is available/);assert.doesNotMatch(practice,/COMMON RESEARCH PRACTICE/);});
