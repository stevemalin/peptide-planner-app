...
test('0.4 customer entry uses established app instead of engineering review shell',()=>{assert.match(app,/export \{default\} from '\.\/App'/);assert.doesNotMatch(app,/MultiPlanLab.*from/);assert.match(customer,/Pep School/);assert.match(customer,/Guide/);assert.match(customer,/My Plans/);assert.match(customer,/Tracker/);assert.match(customer,/More/);});
