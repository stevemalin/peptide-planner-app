...
export default function TodayEventCard({plan,event:e,now,today,active,expanded,onExpand,onAction,onEdit,disabled,children,color}:{plan:SavedPlan;event:Event;now:Date;today:boolean;active:boolean;expanded:boolean;onExpand:()=>void;onAction:(a:'completed'|'skipped'|'later')=>void;onEdit:()=>void;disabled:boolean;children:React.ReactNode;color:string}){
 ...
 const status=done?e.status==='completed'?'Completed':'Skipped':snoozed?'Reminded':canLog?'Due now':today?'Later today':'Scheduled';
 ...
 return <SwipeEvent enabled={today&&active} canLog={canLog} name={plan.compoundName} onComplete={()=>onAction('completed')} onSkip={()=>onAction('skipped')} onLater={()=>onAction('later')} onEdit={onEdit} disabled={disabled}><View testID={done?'completed-event-card':'pending-event-card'} style={[s.card,done&&s.done]}>
 <View style={s.row}><View style={[s.dot,{backgroundColor:color}]}/><Text style={s.name}>{plan.compoundName}</Text><Text style={[s.status,{color:done?'#178066':canLog?'#ac3545':'#61728f'}]}>{status}</Text></View>
 <Text style={s.meta}>{today?'':prettyDate(e.localDate)+' · '}{prettyTime(e.scheduledAt)} · Stage {e.stageIndex+1}{snoozed?' · reminder '+prettyTime(e.snoozedUntil!):''}</Text>
...
const s=StyleSheet.create({card:{padding:10,marginTop:8,borderWidth:1,borderColor:'#dce7f5',borderRadius:16,backgroundColor:'#fff'},done:{backgroundColor:'#f7faf9'},row:{flexDirection:'row',alignItems:'center',gap:6},dot:{width:7,height:7,borderRadius:4},name:{fontSize:15,fontWeight:'800',color:'#10204a',flex:1},status:{fontSize:10,fontWeight:'800'},meta:{fontSize:11,lineHeight:16,color:'#61728f',marginTop:3},amount:{fontSize:14,lineHeight:20,fontWeight:'800',color:'#10204a',marginTop:4},actions:{flexDirection:'row',gap:4,marginTop:2},action:{minHeight:44,flex:1,alignItems:'center',justifyContent:'center',borderRadius:9,backgroundColor:'#eef8fc'},actionText:{fontSize:11,fontWeight:'800',color:'#067da7'},primary:{backgroundColor:'#13ace0'} });
