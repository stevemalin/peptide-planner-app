...
export function reconcileReminders(input:SavedPlan|SavedPlan[]|null):Promise<ReminderReport>{
 const task=serial.catch(()=>{}).then(async()=>{
  ...
  for(const [id,item]of desired)if(!existingIds.has(id))await Notifications.scheduleNotificationAsync({identifier:id,content:{title:item.urgent?'Action needed · '+item.compoundName:'Time for '+item.compoundName,body:item.urgent?'This event is still waiting. Open EZPep Planner to mark Taken, Skip or Remind Later.':'Open EZPep Planner to review the amount and mark Taken, Skip or Remind Later.',sound:'default',data:{owner,eventId:item.eventId,planId:item.planId,urgent:item.urgent}},trigger:{type:Notifications.SchedulableTriggerInputTypes.DATE,date:new Date(item.at),channelId}});
