...
export default function Workspace({screen,navigate,store,update,onGuide}:{screen:WorkspaceScreen;navigate:(screen:WorkspaceScreen)=>void;store:Store;update:(change:(s:Store)=>Store)=>Promise<void>;onGuide:()=>void}){
 ...
 const start=async()=>{
  if(!d||busy)return;setBusy(true);setError('');try{
   if(d.initialVials!==''&&(!Number.isInteger(Number(d.initialVials))||Number(d.initialVials)<0))throw Error('Enter a whole number of individual vials, or leave supply blank.');const plan=activate(d);if(d.reminderEnabled)await enableReminders();
   await save(s=>addPlan({...s,draft:null},plan));navigate('tracker');
  }catch(e){setError(String(e));}finally{setBusy(false);}
 };
 const cloneActive=()=>{const a=store.active;if(!a)return;const {events,activatedAt,inventoryTotalMg,timezone,...fields}=a;save(s=>({...s,draft:{...fields,id:uid(),stages:fields.stages.map(stage=>({...stage,id:uid()})),startDate:'',initialVials:'',reviewed:false}})).then(()=>navigate('plan')).catch(()=>{});};
 ...
 return <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={u.scroll}>
  ...
  {screen==='calc'&&d&&<>
   ...
   <Card><Text style={u.heading}>Supply · optional</Text><Text style={u.small}>Enter individual vials, not kits. 1 kit containing 10 vials = 10 individual vials.</Text><Field label="Number of individual vials" value={d.initialVials} numeric placeholder="Choose… or leave for later" onChange={v=>patch({initialVials:v},false)}/></Card>
   <Card><Text style={u.heading}>{calculationReady?'Ready to start?':'Complete vial setup'}</Text><Text style={u.body}>{totalWeeks} weeks · starts {d.startDate?prettyDate(d.startDate):'Choose…'}</Text><Text style={u.body}>{scheduleSummary(d.defaultSchedule)}</Text>{store.active&&<Text style={u.small}>Your other active plans keep running. Each plan retains its own schedule, supply and history.</Text>}</Card>
