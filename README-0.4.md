...
| Melanotan I / Afamelanotide | Human-study protocol (older subcutaneous research) plus approved controlled-release implant context | No numeric research-vial setup is encoded. | Extract the original subcutaneous study protocol for non-transferable context. Keep the approved implant formulation and duration completely separate from research vials. |
...
| Tesamorelin | Approved-product context plus randomized human subcutaneous protocols in HIV-associated lipodystrophy | No research-vial numeric or vial setup is encoded. | Add the current official label as a separate primary source and extract exact product formulation/population. Do not equate a research vial with the approved product. |
...
| 5-Amino-1MQ | Preclinical evidence; no reliable numeric human source stored | No source links, human amount or vial setup are encoded. | Locate original preclinical papers for non-transferable species/model context. Leave human numeric fields empty. |
| SS-31 / Elamipretide | Approved-label regimen plus randomized human study protocols; indication/formulation specific | Current School links trials only. FDA's 2025 Forzinity label describes a ready-to-use 80 mg/mL solution, not a lyophilized vial requiring reconstitution. | Add the official label as a distinct source after owner review. Keep its Barth-syndrome population, U.S. accelerated-approval status and ready-to-use formulation attached; do not transfer it to a research vial automatically. |
| NAD+ | Human route-specific context is claimed, but no current source links or reliable universal numeric source are stored | No amount, schedule or vial setup is encoded. | Locate original route-specific human studies. Keep intravenous, oral-precursor and other formulations separate; leave universal numeric fields empty. |
| MOTS-c | Human biomarker/exercise observations plus preclinical evidence; no established administered human protocol stored | No source links, amount, schedule or vial setup are encoded. | Locate original human observational and preclinical intervention papers. Do not treat endogenous measurements as an exogenous regimen. |
...
- Current primary-source gaps are most obvious for 5-Amino-1MQ, NAD+, MOTS-c and the approved-label contexts mentioned but not directly linked for tesamorelin, afamelanotide and SS-31.
